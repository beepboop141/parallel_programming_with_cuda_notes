...
// Global queuing kernel - Easier implementation
__global__ void gpu_global_queuing_kernel(
  ...
  for (;;) { //@@ TODO
    int node = currLevelNodes[;;];  //TODO
    for (;;) { // TODO
      int n = nodeNeighbors[;;]; //TODO
...
// Block queuing kernel - A bit more advanced 
__global__ void gpu_block_queuing_kernel(
  ...
  __shared__ int //TODO- Hint: 3 items, one of them is an array

  if (threadIdx.x == 0)
    block_queue_number = 0; 
  //TODO: Hmmm... is something missing?
  ...
  //TODO: What could go here?

  if (threadIdx.x == 0) {
    global_queue_i = // TODO: I want to add the block_queue_number to the numNextLevelNodes
  }
  __syncthreads();
